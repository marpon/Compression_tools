...
private Function Get_Uncomp_File(byref name_file As String, byref fileSize As long ) As ubyte ptr
    Dim fileData As UByte Ptr
    Dim  As Long myHandle = Freefile()
	Dim  As Long  result = Open (name_file For Binary Access Read As #myHandle )
    ...
    If fileSize = 0 Then
       ...
       Exit Function
    End If
    fileData = mem_alloc(sizeof(ubyte), fileSize)
...
private Function Get_Comp_File(byref name_file As String ) As ubyte ptr
    Dim fileData As UByte Ptr
    ...
    Get #myHandle, 0, *fileData , fileSize
    Close #myHandle
	return fileData + sizeof(long) *2
End Function

Private Function Set_Comp_File( Byref File As String, ByVal uComp As Ubyte Ptr) As long
	 if uComp = NULL THEN return -3

	Dim As Long  myHandle , result
	dim as long ulen0 = _len(uComp)
	dim As Ubyte Ptr uComp0 = uComp - sizeof(long) *2
	if ulen0 = 0 THEN return -2
    myHandle = Freefile()
     ...
     If result <> 0 Then
        ...
        Return -1                                 ' error
     End If
     Put #myHandle, , uComp0[0], ulen0 + sizeof(long) *2
     Close #myHandle
     Return 0
End Function


Private Function Set_UnComp_File( Byref File As String, ByVal uComp As Ubyte Ptr) As long
	 if uComp = NULL THEN return -3
...
dim as long fileSize
dim As ubyte ptr in0
in0 =  Get_Uncomp_File("picker.bas", fileSize)

if  in0 = NULL or  fileSize = 0 THEN
	print : print "Failed input file or empty file"
...
print "input size:    " & fileSize & "       in0 ptr " & in0


dim as ubyte ptr enc0 = lzw_encode(in0 , 9)       ' encoding
print "encoded size:  " & _len(enc0)
dim ret1 as long = Set_Comp_File("picker_bas.lzw", enc0 )
 if  ret1 = 0 then
	print: print "Succeeded saving compressed file"
else
	print: print "Failed saving compressed file : " & ret1
end if

dim as ubyte ptr dec0 = lzw_decode(enc0)         ' decoding
print "decoded size:  " & _len(dec0)

if (fileSize <> _len(dec0)) or (memcmp (in0, dec0, fileSize)) <> 0 then
...
ret1 = Set_Uncomp_File("picker_lwz.bas", dec0 )
 if  ret1 = 0 then
	print: print "Succeeded saving uncompressed file"
else
	print: print "Failed saving uncompressed file : " & ret1
end if
