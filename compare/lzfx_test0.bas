'lzfx_test0.bas to test lzfx code
        ...
        while(ip < in_end)
            ...
            if (ctrl < (1 shl 5)) then
                ...
                if (ip + ctrl > in_end) then
					if optional = 0 THEN
						olen =  ilen * 1.25  + 100 / ilen    /'  conservative buffer len '/
						return 0
                    end if
...
print : print "lzfx_getsize  : estimated size for compress buffer  " & i2
...
dim as ubyte ptr dec1 = allocate((i3 ) )    ' add 1 for be able to put 0  at the end
