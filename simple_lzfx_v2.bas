...
'/
	...
	/'  Buffer-to buffer compression.
		...
		estimated required buffer size.  This does not require compression of the entire
	...
	private function size_comp_buffer(byVal ibuf as ubyte ptr , byval ilen as ulong ) as long
		...
		return  ilen * 1.1  + 80 / ilen    /'  conservative buffer len '/
	end function

	private function size_uncomp_buffer(byVal ibuf as ubyte ptr , byval ilen as ulong ) as long
		if ibuf = NULL or  ilen = 0 THEN return LZFX_EARGS
		dim as long temp ,ret1
		ret1 = lzfx_getsize(ibuf , ilen , temp)
		if  ret1 < 0 THEN return ret1
		return 	temp
	end function
 ...
 /'  test code  '/
dim as string sin0 = "abcdefghijklmnopqrstuvwxyz0123456789ZY"'XWVUTSRQPONMLKJIHGFEDCBA"
...
i2 = size_comp_buffer (strptr(sin0) , i0 )
print : print "size_comp_buffer  : estimated for compress buffer  " & i2
...
print enc1[0] , enc1[enc1[0]+2]
dim as long i3 =0

'gives right size  uncompressed needed   to allocate  buffer
i3 = size_uncomp_buffer(enc1 , i2 )
print : print :print "size_uncomp_buffer  : needed for decompress buffer    " & i3
