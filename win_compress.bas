...
'==================================================
#Define _MY__FORMAT_ 		COMPRESSION_FORMAT_XPRESS_HUFF  ' 2 ; 3 or 4
#Define _MY__MAX_			1								' 1 or 0

#define _MY_INPUT_			"decrypt.ico"					' input file name
#define _MY_COMPRESS_		"decrypt_ico.press"				' compressed file name
#define _MY_DECOMPRESS_		"decrypt_dec_ico"				' decompressed file name
'==================================================
...
Function _WinAPI_LZNTDecompress(byval aBinary As Ubyte Ptr , byval iFileSize As Ulong , byval iCompressedSize As Ulong , _
    ...
    dim As Ubyte Ptr pbuffer = callocate(iFileSize , 1)
    Dim As Ulong iUSize
...
Dim As String sFilenameSource = _MY_INPUT_ , sFilenameDest = _MY_COMPRESS_ , sFilenameNew = _MY_DECOMPRESS_

Dim As Ushort iCompressor = _MY__FORMAT_


iFileLenSource = Filelen(sFilenameSource)
iFile = Freefile()
Dim As Ubyte Ptr pMem = Allocate(iFileLenSource) , pMemCompressed
Open sFilenameSource For Binary Access Read As #iFile
Get #iFile , , pMem[0] , iFileLenSource
Close #iFile

pMemCompressed = _WinAPI_LZNTCompress(pMem , iFileLenSource , iCompressedSize , _MY__MAX_ , iCompressor)

 /' iFile = Freefile()
Open sFilenameDest For Binary Access Write As #iFile
Put #iFile, 0, pMemCompressed[0], iCompressedSize
Close #iFile
 '/
