...
if len(command(i)) = 0 or len(command(i + 1)) = 0 then
   print chr(10) ; "ntdll_compress [/D[:buffersize]] [/M] source destination" ; chr(10)
   ...
   print "  /M             Use the maximum compression engine." ; chr(10,10)
...
if decompress then
   ...
   print chr(10) ; "Decompressing " ; command(i) ; " to " ; command(i + 1) ; chr(10)
   rVal = NTDecompressFile(command(i) , command(i + 1) , bufferSize)
   ShowStatus rVal

else
   print chr(10) ; "Compressing " ; command(i) ; " to " ; command(i + 1) ; chr(10)
